...
namespace Swashbuckle.Swagger
{
    public class SchemaRegistry
    {
        ...
        private Schema CreateObjectSchema(JsonObjectContract jsonContract)
        {
            ...
            SetDescription(schema, jsonContract.UnderlyingType);
        ...
        private void SetDescription(Schema schema, Type type)
        {
            var stack = new Stack<Tuple<Schema, Type>>();
            stack.Push(new Tuple<Schema, Type>(schema, type));
            while (stack.Count > 0)
            {
                var item = stack.Pop();
                var sc = item.Item1;
                var tp = item.Item2;
                var customAttribute = tp.GetCustomAttributes(typeof(DescriptionAttribute), true) as DescriptionAttribute[];
                if (customAttribute != null && customAttribute.Length > 0)
                {
                    sc.description = customAttribute[0].Description;
                }
                if (sc.properties != null && sc.properties.Count > 0)
                {
                    var propertys = tp.GetProperties().ToDictionary(k => k.Name, v => v.PropertyType);
                    var types = tp.GetProperties().ToDictionary(k => k.Name, v => v);
                    foreach (KeyValuePair<string, Schema> scInfo in sc.properties)
                    {
                        var info = scInfo.Value;
                        var ca = types[scInfo.Key].GetCustomAttributes(typeof(DescriptionAttribute), true) as DescriptionAttribute[];
                        if (ca != null && ca.Length > 0)
                        {
                            info.description = ca[0].Description;
                        }
                        //stack.Push(new Tuple<Schema, Type>(info, propertys[scInfo.Key]));
                    }
                }
            }
        }
